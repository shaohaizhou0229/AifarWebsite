...
export async function POST(_request, context) {
  try {
    ...
    const task = await cancelSectionRecognitionTask(id, user);
    if (!task) {
      return NextResponse.json({ error: "Recognition task not found or cannot be cancelled." }, { status: 404 });
    }
    ...
    return permissionError(error) || NextResponse.json({ error: error.message || "Could not cancel recognition task." }, { status: 400 });
  }
}
