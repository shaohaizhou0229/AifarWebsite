...
async function enrichUserListRows(pool, rows) {
  if (!rows.length) return [];

  const ids = rows.map((row) => row.id).filter(Boolean);
  const emails = rows.map((row) => String(row.email || "").toLowerCase()).filter(Boolean);
  const [ticketByUserResult, ticketByEmailResult, footprintResult] = await Promise.all([
    ids.length ? pool.query(
      `select user_id, count(*)::int as ticket_count
       from public.contact_requests
       where user_id = any($1::uuid[])
       group by user_id`,
      [ids]
    ) : { rows: [] },
    emails.length ? pool.query(
      `select lower(work_email) as email, count(*)::int as ticket_count
       from public.contact_requests
       where lower(work_email) = any($1::text[])
         and (user_id is null or not (user_id = any($2::uuid[])))
       group by lower(work_email)`,
      [emails, ids]
    ) : { rows: [] },
    ids.length ? pool.query(
      `select user_id, max(created_at) as last_footprint_at
  ...
  const ticketByUser = new Map(ticketByUserResult.rows.map((row) => [row.user_id, Number(row.ticket_count || 0)]));
  const ticketByEmail = new Map(ticketByEmailResult.rows.map((row) => [row.email, Number(row.ticket_count || 0)]));
  const footprintByUser = new Map(footprintResult.rows.map((row) => [row.user_id, row.last_footprint_at]));

  return rows.map((row) => ({
    ...row,
    ticket_count: (ticketByUser.get(row.id) || 0) + (ticketByEmail.get(String(row.email || "").toLowerCase()) || 0),
    last_footprint_at: footprintByUser.get(row.id) || null
  }));
}
...
export async function listAdminUsers(query = "", status = "all", options = {}) {
  ...
  const users = (includeMetrics ? await enrichUserListRows(pool, result.rows) : result.rows).map(mapProfile);
...
export async function getAdminUserMetrics(userIds = []) {
  const ids = [...new Set((Array.isArray(userIds) ? userIds : []).filter(isUuid))].slice(0, 100);
  ...
  const enriched = await enrichUserListRows(pool, result.rows);
  return enriched.map((row) => ({
    id: row.id,
    ticketCount: Number(row.ticket_count || 0),
    lastFootprintAt: row.last_footprint_at || null
  }));
}
