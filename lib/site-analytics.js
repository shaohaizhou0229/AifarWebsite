...
export async function getSiteAnalyticsOverview(days = 7) {
  ...
  try {
    ...
    const [summaryResult, topPagesResult, languageResult, trendResult] = await Promise.all([
      pool.query(
        `select
          ...
          count(*) filter (where event_type = 'download_click')::int as download_clicks
      ...
      pool.query(
        `select
          ...
          count(*) filter (where event_type = 'download_click')::int as downloads
         from public.site_analytics_events
         where created_at >= now() - $1::interval
    ...
    const trend = emptyTrend(safeDays).map((point) => {
      ...
      return row ? { ...point, views: Number(row.views || 0), downloads: Number(row.downloads || 0) } : point;
    });
