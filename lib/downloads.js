...
function mapReleaseVersion(row) {
  if (!row) return null;
  return {
    id: row.id,
    platform: row.platform,
    ...
    isPublished: Boolean(row.is_published),
    publishedAt: row.published_at,
    snapshotType: row.snapshot_type || "updated",
    createdBy: row.created_by,
    createdAt: row.created_at,
...
async function recordClientReleaseVersion(platformKey, adminUser, snapshotType, release) {
  if (!release) return null;
  const pool = getPostgresPool();
  const result = await pool.query(
    `insert into public.client_release_versions (
      platform,
      version,
      build_number,
      release_notes,
    ...
    [
      ...
      release.uploadStatus || "idle",
      release.originalFilename || "",
      release.contentType || "",
      Boolean(release.isPublished),
      release.publishedAt || null,
      snapshotType,
      adminUser?.id || null
    ]
  );
  return mapReleaseVersion(result.rows[0]);
}
...
export async function listPublicDownloadPlatforms() {
  try {
    ...
    return DOWNLOAD_PLATFORMS.map((platform) => mergePlatform(platform, null));
  }
}
...
export async function listClientReleaseVersions(platformKey, limit = 20) {
  const safePlatform = sanitizePlatform(platformKey);
  if (!safePlatform) return [];

  const safeLimit = Math.min(Math.max(Number(limit || 20), 1), 50);
  const pool = getPostgresPool();
  const result = await pool.query(
    `select versions.*, profiles.display_name as actor_name, profiles.email as actor_email
     from public.client_release_versions versions
     left join public.profiles profiles on profiles.id = versions.created_by
     where versions.platform = $1
     order by versions.created_at desc
     limit $2`,
    [safePlatform, safeLimit]
  );
...
export async function updateClientRelease(platformKey, adminUser, input) {
  ...
  const release = mapRelease(result.rows[0]);
  await recordClientReleaseVersion(platformKey, adminUser, release.isPublished ? "published" : "details_saved", release);
  return release;
}
...
export async function updateClientReleaseFile(platformKey, adminUser, fileInfo) {
  ...
  const release = mapRelease(result.rows[0]);
  await recordClientReleaseVersion(platformKey, adminUser, "file_uploaded", release);
  return release;
}
...
export async function markClientReleaseUploading(platformKey, adminUser, fileInfo) {
  ...
  const release = mapRelease(result.rows[0]);
  await recordClientReleaseVersion(platformKey, adminUser, "upload_started", release);
  return release;
}
...
export async function updateClientReleaseUploadStatus(platformKey, adminUser, uploadStatus) {
  ...
  const release = mapRelease(result.rows[0]);
  await recordClientReleaseVersion(platformKey, adminUser, `upload_${uploadStatus}`, release);
  return release;
}
...
export async function clearClientReleaseFile(platformKey, adminUser) {
  ...
  const release = mapRelease(result.rows[0]);
  await recordClientReleaseVersion(platformKey, adminUser, "file_cleared", release);
  return release;
}

export async function restoreClientReleaseVersion(platformKey, versionId, adminUser) {
  const safePlatform = sanitizePlatform(platformKey);
  ...
  const versionResult = await pool.query(
    `select *
     from public.client_release_versions
     where id = $1 and platform = $2
     limit 1`,
    [versionId, safePlatform]
  );
  ...
  const release = mapRelease(result.rows[0]);
  await recordClientReleaseVersion(safePlatform, adminUser, "restored", release);
  return release;
}
