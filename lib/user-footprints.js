...
export const USER_FOOTPRINT_EVENTS = {
  ...
  emailChangeRequested: "profile.email_change_requested",
  passwordUpdated: "profile.password_updated",
  googleUnlinked: "profile.google_unlinked",
  adminUserUpdated: "admin.user_updated",
  adminUserDeleted: "admin.user_deleted",
  adminTestAccountReset: "admin.test_account_reset",
  adminUserInvited: "admin.user_invited",
  adminInvitationCanceled: "admin.invitation_canceled",
...
export async function listRecentUserFootprints(limit = 20) {
  const pool = getPostgresPool();
  const result = await pool.query(
    `select f.*, p.display_name as actor_name, p.email as actor_email
     from public.user_footprints f
