...
const AI_LAYOUT_SHADOWS = ["none", "subtle", "medium", "strong"];
const AI_LAYOUT_HOVERS = ["none", "lift", "glow", "dim"];
const AI_LAYOUT_BORDERS = ["none", "subtle", "strong"];
const AI_LAYOUT_GAPS = ["compact", "normal", "relaxed"];
...
function normalizeElementStyle(value = {}) {
  ...
  const normalized = {};
  const allowedKeys = new Set(["textSize", "weight", "align", "tone", "radius", "variant", "fill", "padding", "fit", "lineClamp", "shadow", "hover", "border", "gap"]);
  ...
  for (const key of Object.keys(value)) {
    ...
    if (key === "shadow") normalized.shadow = token(value[key], AI_LAYOUT_SHADOWS, "");
    if (key === "hover") normalized.hover = token(value[key], AI_LAYOUT_HOVERS, "");
    if (key === "border") normalized.border = token(value[key], AI_LAYOUT_BORDERS, "");
    if (key === "gap") normalized.gap = token(value[key], AI_LAYOUT_GAPS, "");
  }
...
function normalizeAiLayoutElement(element, index = 0) {
  ...
  if (isObject(element.originalBox)) normalized.originalBox = normalizeBox(element.originalBox, normalized.box);
  if (element.locked === true) normalized.locked = true;
  const label = safeText(element.label || element.name, `aiLayout.elements[${index}].label`, 120);
  if (label) normalized.label = label;
  const groupId = safeId(element.groupId || element.group, "");
