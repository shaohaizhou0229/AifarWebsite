...
export function buildNotificationEmail({ title, body, actionUrl }) {
  const lines = [
    body,
    actionUrl
  ];
  ...
  return {
    subject: `Aifar 通知 / Notification: ${title}`,
    text: textBlock(lines),
    html: layout(title, paragraph(lines.slice(0, -1)) + button("打开 Aifar / Open Aifar", actionUrl))
  };
}
