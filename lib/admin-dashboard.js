...
export async function getAdminDashboardOverview({ userId, analyticsDays = 7 }) {
  const safeAnalyticsDays = Number(analyticsDays) === 1 ? 1 : 7;
  const [analytics, platforms, contactTickets, supportTickets, contactStats, supportStats, userOverview, documents, subtasks, footprints, productDrafts, productActivity] = await Promise.all([
