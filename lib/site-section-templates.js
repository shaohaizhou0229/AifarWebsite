...
import sectionTemplateStorageRules from "@/lib/section-template-storage-rules.cjs";
...
export async function createSiteSectionTemplateRecord(adminUser, input = {}) {
  ...
  const template = sectionTemplateStorageRules.prepareSectionTemplateCreateInput(input);
  const pool = getPostgresPool();
  const result = await pool.query(
...
export async function updateSiteSectionTemplateRecord(templateId, adminUser, input = {}) {
  ...
  const template = sectionTemplateStorageRules.prepareSectionTemplateMetadataUpdateInput(current, input);
  const pool = getPostgresPool();
  const result = await pool.query(
