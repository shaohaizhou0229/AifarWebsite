...
export function mergeSiteContent(pageKey, fallback, override) {
  const content = override && typeof override === "object" && !Array.isArray(override) ? override : {};
  const merged = {
    ...fallback,
    ...content,
  ...
  if (pageKey === "home") {
    ...
    merged.updates = fallback.updates;
    merged.ctaEyebrow = fallback.ctaEyebrow;
    merged.ctaTitle = fallback.ctaTitle;
    merged.ctaLead = fallback.ctaLead;
    merged.ctaPrimary = fallback.ctaPrimary;
    merged.ctaSecondary = fallback.ctaSecondary;
  }
  ...
  merged.heroImageUrl = getSiteImageUrl(merged.heroImagePath) || fallback.heroImageUrl || "/assets/images/aifar-hero.png";
  return merged;
}
  ...
  const result = await pool.query(
    `update public.site_page_contents
     set
      ...
      hero_image_path = nullif(draft_content->>'heroImagePath', ''),
      hero_image_alt = nullif(draft_content->>'heroAlt', ''),
      is_published = true,
      published_at = now(),
      updated_by = $3,
      updated_at = now()
     where page_key = $1 and locale = $2
     returning *`,
    [safePageKey, safeLocale, adminUser.id]
  );
...
export async function isPublishedSiteImage(storagePath) {
  ...
  const result = await pool.query(
    `select 1
     from public.site_page_contents
     where hero_image_path = $1 and is_published = true
     limit 1`,
    [storagePath]
  );
