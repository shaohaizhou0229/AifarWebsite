...
const DEFAULT_SILICONFLOW_RECOGNITION_TIMEOUT_MS = 120000;
...
function createFallbackAiLayoutElements(section = {}) {
  const content = section.content && typeof section.content === "object" && !Array.isArray(section.content)
    ? section.content
    : {};
  const elements = [];

  if (clean(content.eyebrow)) {
    elements.push({
      id: "badge-1",
      type: "badge",
      text: content.eyebrow,
      box: { x: 0.42, y: 0.08, width: 0.16, height: 0.05 },
      zIndex: 2,
      appearance: { align: "center", tone: "brand", textSize: "xs", weight: "700", radius: "large" }
    });
  }
  if (clean(content.title)) {
    elements.push({
      ...
      type: "text",
      text: content.title,
      box: { x: 0.22, y: 0.16, width: 0.56, height: 0.14 },
      zIndex: 2,
      appearance: { align: "center", tone: "default", textSize: "xl", weight: "800" }
    });
  }
  if (clean(content.lead)) {
    elements.push({
      id: "lead-1",
      type: "text",
      text: content.lead,
      box: { x: 0.24, y: 0.32, width: 0.52, height: 0.1 },
      zIndex: 2,
      appearance: { align: "center", tone: "muted", textSize: "sm", weight: "500" }
    });
  }
  if (clean(content.primaryCta || content.actionLabel)) {
    elements.push({
      id: "button-1",
      type: "button",
      text: content.primaryCta || content.actionLabel,
      href: content.primaryHref || content.actionHref || "",
      box: { x: 0.43, y: 0.46, width: 0.14, height: 0.07 },
      zIndex: 2,
      appearance: { align: "center", tone: "brand", textSize: "sm", weight: "700", radius: "medium" }
    });
  }
  if ("heroImagePath" in content || "imagePath" in content || clean(content.heroAlt || content.imageAlt)) {
    elements.push({
      id: "image-1",
      type: "image",
      imagePath: "",
      imageUrl: "",
      alt: content.heroAlt || content.imageAlt || "Recognized screenshot image placeholder",
      box: { x: 0.18, y: 0.6, width: 0.64, height: 0.28 },
      zIndex: 1,
  ...
  return elements.length ? elements : [{
    id: "title-1",
    type: "text",
    text: "AI recognized section",
    box: { x: 0.24, y: 0.2, width: 0.52, height: 0.16 },
    zIndex: 2,
    appearance: { align: "center", tone: "default", textSize: "xl", weight: "800" }
  }];
}
...
function coerceRecognizedAiLayoutSection(section = {}, recognition = {}) {
  ...
  if (type === AI_LAYOUT_SECTION_TYPE) return section;
  ...
  return {
    ...
    content: {
      ...
      elements: Array.isArray(recognition.detectedElements) && recognition.detectedElements.length
        ? recognition.detectedElements
        : createFallbackAiLayoutElements(section)
    }
  };
}
...
function normalizeRecognitionCandidate(result, fields = {}, sourceImage = {}) {
  ...
  const section = normalizeRecognizedSettingsShape(sanitizeExternalLinks(
    coerceRecognizedAiLayoutSection(result.template.section, result.recognition),
    linkState
  ));
  ...
  const detectedElements = normalizeAiLayoutElements(result.recognition.detectedElements || section.content?.elements || []);
