...
function getSectionTemplateRecognitionUatMode(env = process.env) {
  const uatModeRequested = isTruthyFlag(env.OPENAI_SECTION_TEMPLATE_UAT_MODE);
  ...
  if (normalizedType === "hero") {
    return {
      ...
      content: {
        ...
        lead: "This block tests upload, preview, save, insert, and settings flows without calling OpenAI.",
...
function buildUatRecognitionResult(fields = {}) {
  ...
  return {
    ...
    recognition: {
      confidence: 0.62,
      notes: [
        "Local UAT mode generated this candidate without calling OpenAI.",
