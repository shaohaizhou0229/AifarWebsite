...
export const CONTACT_REQUEST_TYPES = new Set(["product_inquiry", "partnership", "other"]);
export const SUPPORT_REQUEST_TYPES = new Set(["technical_support", "account_access", "client_download", "installation", "product_usage", "bug_report"]);
export const PUBLIC_REQUEST_TYPES = new Set([...CONTACT_REQUEST_TYPES, ...SUPPORT_REQUEST_TYPES]);

export function getRequestWorkflow(requestType) {
  return SUPPORT_REQUEST_TYPES.has(requestType) ? "support" : "contact";
}

export function getCategoryForRequestType(requestType) {
  if (requestType === "partnership") return "partnership";
  if (requestType === "technical_support") return "product_usage";
  if (TICKET_CATEGORIES.has(requestType)) return requestType;
  return "other";
}
...
export async function listAdminTickets(input = {}) {
  ...
  if (filters.scope === "contact") {
    params.push([...CONTACT_REQUEST_TYPES]);
    where.push(`cr.request_type = any($${params.length})`);
  } else if (filters.scope === "support") {
...
export async function getAdminTicketStats() {
  return getAdminTicketStatsByScope("support");
}

export async function getAdminTicketStatsByScope(scope = "") {
  const pool = getPostgresPool();
  const params = [];
  const where = [];
  if (scope === "contact") {
    params.push([...CONTACT_REQUEST_TYPES]);
    where.push(`request_type = any($${params.length})`);
