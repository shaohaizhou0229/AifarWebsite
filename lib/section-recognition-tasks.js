...
const TASK_AI_TIMEOUT_MS = 120000;
  ...
  if (settings.providerKey === "siliconflow") {
    return fetchRecognitionJson({
      ...
      body: buildSiliconFlowDirectRecognitionPayload({
        model: settings.visionModel || settings.modelIds?.[0] || settings.model,
...
export async function processSectionRecognitionTask(taskId) {
  ...
  try {
    ...
    if (settings.uatModeEnabled) {
      return completeTask(task.id, createUatRecognitionOutput(fields, sourceImageFromTask(task)));
    }
    ...
    try {
      ...
      const output = normalizeRecognitionCandidate(result, fields, sourceImageFromTask(task));
