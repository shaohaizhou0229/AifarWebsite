...
function shouldRepairRecognitionJson(error) {
  return error?.code === "invalid_recognition_json" && Boolean(error.rawText);
}

function createUnrepairableJsonError() {
  const error = new Error("AI returned incomplete JSON. Please retry or use a clearer screenshot.");
  error.code = "invalid_recognition_json";
  return error;
}

async function requestRepairedRecognitionJson({ fields, settings, parseError, signal }) {
  if (settings.providerKey === "siliconflow") {
    const model = settings.textModel || settings.visionModel || settings.modelIds?.[0] || settings.model;
    return fetchRecognitionJson({
      url: `${settings.baseUrl}/chat/completions`,
      apiKey: settings.apiKey,
      signal,
      body: buildSiliconFlowJsonRepairPayload({
        model,
  ...
  return fetchRecognitionJson({
    ...
    apiKey: settings.apiKey,
    signal,
    body: buildOpenAIJsonRepairPayload({
      model: settings.model,
      rawText: parseError.rawText,
      parseError,
      fields
...
export async function processSectionRecognitionTask(taskId) {
  ...
  try {
    ...
    try {
      ...
      let result;
      try {
        result = extractResponseJson(responseJson);
      } catch (parseError) {
        if (!shouldRepairRecognitionJson(parseError)) throw parseError;
        const repairResponseJson = await requestRepairedRecognitionJson({
          fields,
          settings,
          parseError,
          signal: controller.signal
        ...
        try {
          result = extractResponseJson(repairResponseJson);
        } catch {
          throw createUnrepairableJsonError();
        }
      }
