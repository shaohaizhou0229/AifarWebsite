...
async function assertSubtaskAccess(subtaskId, userId) {
  const pool = getPostgresPool();
  const result = await pool.query(
    `select st.*, t.space_id, s.leader_user_id, m.member_role
  ...
  return {
    subtask: row,
    isLeader: row.leader_user_id === userId || row.member_role === "leader",
    isAssignee: row.assignee_user_id === userId
  };
}
...
export async function createCollaborationSubtask(taskId, input, actorUserId, locale = "zh-CN") {
  ...
  if (assigneeUserId) {
    await createNotification({
      ...
      metadata: { taskId },
      url: `${process.env.NEXT_PUBLIC_SITE_URL || ""}/${locale}/admin/collaboration/spaces/${task.space_id}/`
    });
  }
  return subtask;
}
...
export async function updateCollaborationSubtask(subtaskId, input, actorUserId, locale = "zh-CN") {
  const { subtask, isLeader, isAssignee } = await assertSubtaskAccess(subtaskId, actorUserId);
  if (!isLeader && !isAssignee) {
    throw new Error("Only the leader or assignee can update this subtask.");
  }
  ...
  if (updated.assigneeUserId && ["blocked", "completed"].includes(status)) {
    await createNotification({
      ...
      title: status === "blocked" ? "协作子任务已阻塞" : "协作子任务已完成",
      body: `子任务「${updated.title}」状态已更新。`,
      relatedType: "collaboration_subtask",
      relatedId: updated.id,
      metadata: { status },
      url: `${process.env.NEXT_PUBLIC_SITE_URL || ""}/${locale}/admin/collaboration/spaces/${subtask.space_id}/`
    });
  }
  return updated;
}
