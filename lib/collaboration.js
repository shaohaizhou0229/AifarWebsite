...
export const TASK_STATUSES = coreRules.TASK_STATUSES;
export const TASK_TYPES = coreRules.TASK_TYPES;
export const REPEAT_FREQUENCIES = coreRules.REPEAT_FREQUENCIES;
...
function nullableText(value) {
  return coreRules.nullableText(value);
}

function nullableDate(value) {
  return coreRules.nullableDate(value);
}

function normalizeStatus(value) {
  return coreRules.normalizeTaskStatus(value);
}

function normalizeTaskType(value) {
  return coreRules.normalizeTaskType(value);
}

function normalizeRepeatFrequency(value, taskType) {
  return coreRules.normalizeRepeatFrequency(value, taskType);
}
...
export function canCreateCollaborationSpace(profile) {
  return coreRules.canCreateCollaborationSpace(profile);
}
...
export async function addCollaborationMember(spaceId, memberUserId, actorUserId, locale = "zh-CN") {
  ...
  await createNotification({
    ...
    relatedId: spaceId,
    url: `/${locale}/admin/collaboration/spaces/${spaceId}/`
  });
...
export async function createCollaborationSubtask(taskId, input, actorUserId, locale = "zh-CN") {
  ...
  if (assigneeUserId) {
    await createNotification({
      ...
      metadata: { taskId },
      url: `/${locale}/admin/collaboration/subtasks/${subtask.id}/`
    });
  }
  return subtask;
}
...
export async function updateCollaborationSubtask(subtaskId, input, actorUserId, locale = "zh-CN") {
  ...
  if (updated.assigneeUserId && ["blocked", "completed"].includes(status)) {
    await createNotification({
      ...
      metadata: { status },
      url: `/${locale}/admin/collaboration/subtasks/${updated.id}/`
    });
  }
  return updated;
}
    ...
    try {
      ...
      for (const member of members.rows) {
        await createNotification({
          ...
          url: `/${locale}/admin/collaboration/tasks/${newTask.id}/`
        });
      }
    } catch (error) {
      await client.query("rollback");
